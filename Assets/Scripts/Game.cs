...
//		}
	...
	public void OnTouchZone()
	{
		if(!isTouchedZone)
			StartCoroutine (touchZone ());
	}

	IEnumerator touchZone()
	{
		isTouchedZone = true;
		touchZoneLbl.SetActive (true);
		yield return new WaitForSeconds (2.5f);
		touchZoneLbl.SetActive (false);
		BikeManager.instance.OnReset ();
		isTouchedZone = false;
		yield return null;
	}
